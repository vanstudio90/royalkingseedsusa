    ...
    <div className="product-card group">
      <Link href={`/${product.slug}`}>
        <div className="product-image aspect-square bg-white flex items-center justify-center relative mb-3 sm:mb-5 overflow-hidden">
          {product.imageUrl ? (
            <img src={product.imageUrl} alt={product.name} loading="lazy" decoding="async" className="w-full h-full object-contain p-1 sm:p-3 group-hover:scale-105 transition-transform duration-300" />
          ) : (
          ...
          <button
            ...
            className={`absolute bottom-3 right-3 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 cursor-pointer ${
              isInWishlist ? 'bg-red-500 text-white shadow-md scale-110' : 'bg-white/80 text-[#192026]/50 hover:text-red-400 hover:bg-white shadow-sm'
            }`}
        ...
        <div className="mt-2 sm:mt-3 px-1" onClick={(e) => e.preventDefault()}>
          <div className="relative">
            <select
              ...
              onChange={(e) => { e.preventDefault(); e.stopPropagation(); setSelectedOption(Number(e.target.value)); }}
            ...
            >
              {seedOpts.map((opt, i) => (
                <option key={i} value={i}>{opt.label} — ${opt.price.toFixed(2)}</option>
              ))}
            </select>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mt-3 sm:mt-5 pt-3 sm:pt-4 border-t border-[#F5F0EA] gap-2">
        ...
        <button
          ...
          className={`px-3 sm:px-4 py-1.5 sm:py-2 text-white text-[10px] sm:text-[11px] uppercase tracking-[1px] font-medium rounded-full transition-colors duration-300 cursor-pointer ${
            added ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-[#275C53] hover:bg-[#1e4a42]'
          }`}
        >

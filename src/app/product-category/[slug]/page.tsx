...
function filterProductsByCategory(products: any[], slug: string) {
  ...
  const filters: Record<string, (p: any) => boolean> = {
    ...
    'best-strains-for-high-yield': (p) => p.categories.some((c: string) => /high.?yield|big.?yield/i.test(c)) || /big bud|critical|super skunk|northern lights|white widow|blue dream|amnesia|gorilla|bruce banner|wedding cake|girl scout|green crack|jack herer|granddaddy|cheese|ak.?47|power plant/i.test(p.name),
    'best-strains-for-indoor-growing': (p) => p.categories.some((c: string) => c.toLowerCase().includes('indoor')) || (p.strainType === 'indica' || p.strainType === 'hybrid') && !p.name.toLowerCase().includes('outdoor'),
